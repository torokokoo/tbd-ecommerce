    ...
    <div v-else class="ordenes-list">
      <div v-for="orden in ordenes" 
        ...
        <div class="orden-header">
          <button 
            class="delete-btn"
            @click.stop="eliminarOrden(orden.idOrden)"
...
const ESTADOS_VALIDOS = ['PENDIENTE'];
...
const eliminarOrden = async (idOrden) => {
  try {
    ...
    const detallesResponse = await detalleOrdenService.getDetallesByOrden(idOrden);
    for (const detalle of detallesResponse.data) {
      await detalleOrdenService.deleteDetalleOrden(detalle.idDetalle);
    }

    // Luego eliminar la orden
    await ordenService.deleteOrden(idOrden);

    // Actualizar la lista de órdenes
    ordenes.value = ordenes.value.filter(orden => orden.idOrden !== idOrden);

    // Mostrar mensaje de éxito temporal
    const tempDiv = document.createElement('div');
    tempDiv.className = 'success-message';
    tempDiv.textContent = 'Orden eliminada con éxito';
    document.querySelector('.create-tarea').prepend(tempDiv);
    setTimeout(() => tempDiv.remove(), 3000);

  } catch (err) {
    console.error('Error al eliminar la orden:', err);
    error.value = 'Error al eliminar la orden. Por favor, intente nuevamente.';
    setTimeout(() => error.value = null, 3000);
  }
};

// Agregar esta función en la sección de script
const pagarOrden = async (orden) => {
  try {
    if (!confirm('¿Está seguro de pagar esta orden?')) {
      return;
    }
    ...
    await ordenService.updateOrden(orden.idOrden, ordenActualizada);

    // Actualizar la orden en el estado local
    const index = ordenes.value.findIndex(o => o.idOrden === orden.idOrden);
    if (index !== -1) {
      ordenes.value[index] = ordenActualizada;
    }

    // Eliminar la orden de la lista si ya no cumple con los estados válidos
    if (!ESTADOS_VALIDOS.includes(ordenActualizada.estado.toUpperCase())) {
      ordenes.value = ordenes.value.filter(o => o.idOrden !== orden.idOrden);
    }

    // Mostrar mensaje de éxito
    const tempDiv = document.createElement('div');
    tempDiv.className = 'success-message';
    tempDiv.textContent = 'Orden pagada con éxito';
    document.querySelector('.create-tarea').prepend(tempDiv);
    setTimeout(() => tempDiv.remove(), 3000);

  } catch (err) {
    console.error('Error al pagar la orden:', err);
    error.value = 'Error al procesar el pago. Por favor, intente nuevamente.';
    setTimeout(() => error.value = null, 3000);
  }
};

// Actualizar la función getEstadoClase para incluir el nuevo estado
